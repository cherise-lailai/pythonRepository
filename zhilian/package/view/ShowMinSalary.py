#python与java岗位在福州的最低薪水折线图
from pyecharts import Line
from package.wash import DataWash
import random
javajobNameList,javaMinSalaryList,pythonjobNameList,pythonMinSalaryList=DataWash.washMinSalary()
...
for i in range(len(javajobNameList)):
    list.append((javajobNameList[i],javaMinSalaryList[i]))
showJavaJob=[]
...
for i in range(len(pythonjobNameList)):
    list.append((pythonjobNameList[i],pythonMinSalaryList[i]))
showPythonJob=[]
for i in range(showNum):
...
line.render("minSalaryLineChart.html")